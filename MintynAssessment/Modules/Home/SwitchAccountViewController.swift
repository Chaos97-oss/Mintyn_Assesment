...
class SwitchAccountViewController: FeatureBaseViewController {
    init() { super.init(title: "Switch Account", icon: "person.2.circle.fill") }
    required init?(coder: NSCoder) { fatalError("init(coder:) has not been implemented") }
    override func viewDidLoad() {
        super.viewDidLoad()
        let desc = UILabel()
        desc.text = "Manage your alternative profiles here."
